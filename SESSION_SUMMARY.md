...
## Continuation Session — March 27–28, 2026

### Overview
Focused on project tooling and codebase health: consolidated Claude skills, added session handoff system, ran a full dead code audit across all JS files, cleaned up all confirmed dead code, audited file sizes, and reviewed unimplemented perks.

---

### SKILL CONSOLIDATION

**Merged two skills into one:**
- Old: `tech-warrior` (project architecture knowledge) + `code-session-prompter` (Claude Code prompt patterns)
- New: single `tech-warrior` skill covering both

**New skill philosophy:**
- Does NOT contain game-specific details (weapons, stats, tokens) — those change too fast
- Teaches where the 3 source-of-truth files are and what each covers
- Teaches reading order, session workflow, architecture rules
- Contains all Claude Code prompt patterns (anti-stall, chunking, audit/fix separation, templates)
- Session management rules (one branch per task, merge before next, etc.)

**Discrepancy fixed:** Old skill said version lives in OVERVIEW.md — corrected to `GAME_VERSION` in `js/constants.js` only (per CLAUDE.md).

**Source-of-truth file update guidance added:**
- OVERVIEW.md: update every code-changing session (already existed)
- CLAUDE.md: update when session adds new architecture rules, naming conventions, or "DO NOT" items
- UI_CONVENTIONS.md: update when session changes design tokens, color semantics, slot labels, layout, or UI patterns
- Both prompt templates updated to include this check

**Session Handoff File (SESSION_LOG.md) added to skill:**
- Chat-to-chat handoff — written at end of a long chat session on request
- Saved and uploaded to next chat to resume context
- Sections: What Was Done, Claude Code Prompts Written, Key Decisions, Current State, Next Steps, Open Questions
- Overwrites each time — only latest session matters
- Distinct from OVERVIEW.md: captures session narrative and continuity, not structural state

---

### DEAD CODE AUDIT — COMPLETE

**5 audit-only sessions run across all JS files:**
1. Core Systems (constants.js, state.js, utils.js, audio.js)
2. Combat & Mechs (mechs.js, cover.js, combat.js, mods.js)
3. Enemies, Rounds & Perks (perks.js, enemies.js, rounds.js, enemy-types.js, arena-objectives.js)
4. UI & Menus (hud.js, garage.js, menus.js, loot-system.js)
5. Campaign, Skill Tree, Multiplayer & Events (campaign-system.js, skill-tree-data.js, skill-tree.js, multiplayer.js, events.js, init.js)

**Results: 37 HIGH confidence, 3 MEDIUM, 0 LOW**

**Major findings:**
- ~73 unimplemented perks (players could pick them but they did nothing)
- Dead item-detail chain in menus.js (4 functions, replaced by hover cards)
- Dead loadout-slots subsystem in campaign-system.js (save/load/delete presets — no entry point)
- `if(false)` block in showShop() (~72 lines of disabled buy-detail panel)
- Dead leaderboard stubs (submitLeaderboardEntry, skipLeaderboardSubmit)
- Various unused functions, variables, and constants across 7 other files

**Clean files (no dead code found):** enemies.js, rounds.js, enemy-types.js, arena-objectives.js, multiplayer.js, events.js, init.js

---

### DEAD CODE CLEANUP — COMPLETE

**3 cleanup sessions + 1 perk cleanup session, all merged to main:**

Session 1 — campaign-system.js + menus.js:
- Removed `if(false)` buy-detail block, `_shopItemTotal`, `_shopSelect`
- Removed entire loadout-slots subsystem (7 functions + DOM element)
- Removed `_showCloudStatusToast` no-op stub + 3 call sites
- Removed dead item-detail chain (4 functions) + `#inv-detail-panel` DOM element
- Removed `submitLeaderboardEntry` + `skipLeaderboardSubmit`

Session 2 — loot-system.js + skill-tree.js + garage.js + hud.js:
- Removed `triggerEchoStrike` and `checkMirrorShot` stubs
- Removed `_lockedAllocations`, `_isNodeAvailable`, `_skillTreeClampVB`
- Removed `_calcWeight`
- Removed unused `lootDef` local variable

Session 3 — constants.js + state.js + combat.js + mechs.js + cover.js + mods.js + audio.js:
- Removed `window.TW = {}`, `ENEMY_PRIMARY`, `ENEMY_ARM_WEAPONS`
- Removed `_pendingLoadoutTab`
- Removed orphaned `_perkState` fields (perfectAccuracy, reinforcedCore, fthDmg)
- Removed `_createAfterburn`, `player._siphonLine`, `tryPlace`, `decoyFireTimer`, unused `partName` destructure
- Removed unreachable `case 'missile':` in sndFire

Perk Cleanup Session:
- Removed ~62 unimplemented perks + their `_perkState` fields from perks.js and state.js
- Kept 11 perks hidden from selection pool via `_hiddenPerks` Set for future implementation:
  `ricochet_rounds`, `smg_ricochet`, `mg_explosive_tips`, `sg_incendiary`, `br_crit_burst`, `hr_concussive`, `sr_double_shot`, `gl_toxic`, `rl_napalm`, `plsm_gravity`, `rail_chain_lightning`

---

### FILE SIZE AUDIT — COMPLETE

**Total codebase: 30,881 lines / ~314K tokens across 23 JS files**

**Files flagged LARGE (>1500 lines):**

| File | Lines | Tokens | Natural Split Points Identified |
|------|-------|--------|-------------------------------|
| skill-tree-data.js | 8,467 | 35,300 | Pure data — split by chassis (deferred, no logic benefit) |
| multiplayer.js | 2,660 | 28,900 | Lobby/chat, match gameplay, PVP hangar |
| menus.js | 2,550 | 30,600 | Inventory/loadout/hover cards, leaderboard, death/pause overlays |
| enemies.js | 2,431 | 29,800 | Boss spawners (~900 lines), AI state machine |
| combat.js | 1,945 | 22,900 | Player damage/shields, enemy damage/bullets |
| loot-system.js | 1,902 | 21,200 | Unique item effects, item generation |
| campaign-system.js | 1,812 | 20,200 | Supply shop (~500 lines), mission select UI |

**Notable:** perks.js (1,038 lines) has highest token density of any logic file (32,100 tokens) due to perk dictionary strings.

**Decision: No splits for now.** Split points documented in FILE_SIZE_AUDIT.md for future use.
...
| File | Purpose |
|------|---------|
| SKILL.md | Consolidated tech-warrior skill (replaces tech-warrior + code-session-prompter) |
| DEAD_CODE_AUDIT.md | Full audit findings across all 23 JS files |
| DEAD_CODE_AUDIT_PROMPTS.md | 5 audit-only Claude Code prompts |
| DEAD_CODE_CLEANUP_PROMPTS.md | 3 cleanup Claude Code prompts |
| PERK_CLEANUP_PROMPT.md | Perk removal/hiding Claude Code prompt |
| FILE_SIZE_AUDIT_PROMPT.md | File size audit Claude Code prompt |
| FILE_SIZE_AUDIT.md | File size results with split point analysis |
| UNIMPLEMENTED_PERKS_REVIEW.md | Perk review checklist (used, then acted on) |

---

### NEXT STEPS

- Implement the 11 hidden perks when ready (they're in perks.js behind `_hiddenPerks` Set)
- Consider file splits if any LARGE file becomes painful during Claude Code sessions
- Shield bug prompt from previous session still pending (new campaigns start without shields)
- Item comparison bugs prompt from previous session still pending (missing comparison + affix stats)
