...
Last updated: March 23, 2026 (v5.84 — left column reorder, hover card overhaul, backpack hover)
...
| `js/menus.js` | All menu screen logic: main menu nav (`proceedToMainMenu`, `showCampaignSubMenu`, `hideCampaignSubMenu`, `returnToMainMenu`), hangar nav (`returnToHangar`, `returnToHangarForMissionSelect`, `deployMech`, `startGame`, `startMultiplayer`, `goToMainMenu`), death screen (`showDeathScreen`, `respawnMech`), pause (`togglePause`), loadout overlay (`toggleStats`, `populateLoadout`, `populateInventory`, `_renderHullBars`, `_renderGearBonusesPanel`, `_renderActivePerksPanel`, `_renderWeaponBar`, `_showSlotHover`, `_hideSlotHover`, item detail `_showItemDetail`, `_renderItemDetail`, `_buildItemComparisonHTML`, `_setCompareArm`), leaderboard (`showLeaderboard`, `closeLeaderboard`, `submitLeaderboardEntry`), and campaign chassis select flow. |
