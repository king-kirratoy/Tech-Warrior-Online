...
Last updated: March 24, 2026 (v6.17 — BUILD STATS cleanup: remove gap bars, collapse detail rows, uppercase dropdowns)
...
| `js/garage.js` | Warzone hangar UI and loadout management. `_wzToggleDD()`, `_wzBuildDropdown()`, `_wzCloseAllDD()`, `_wzBuildColorDD()`, `_wzToggleColorDD()` (warzone dropdown system using mp-dd-*/wz-dd-* classes), `selectSlot()` (slot selection with 2H weapon locking), `refreshGarage()` (full dynamic renderer — rebuilds entire warzone hangar HTML including dropdowns, preview, and build stats), `_wzGetSlotLabel()`, `setChassis()`, `_calcWeight()`. Legacy wrappers `toggleDD()`/`closeAllDD()` kept for external callers. |
