...
| `js/constants.js` | All immutable game data: chassis, weapons (including `siphon` beam weapon — added to `WEAPON_OPTIONS` in v6.69 so it appears in warzone/PVP dropdowns), shields (11 total: 4 light, 4 medium including `fortress_shield`, 3 heavy), augments (12 total: none + 11 active — 20 removed in v6.68), legs (13 total: 5 universal + 3 light + 3 medium + 2 heavy), cover, arenas, enemy color palettes, restriction Sets, starter loadouts, `SLOT_ID_MAP`, and `GAME_CONFIG`. |
