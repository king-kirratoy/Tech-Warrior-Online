...
| `js/skill-tree.js` | Skill tree overlay: `showSkillTree()` / `hideSkillTree()`, pan/zoom SVG canvas, bottom legend, `_renderSkillTree()` (SVG connection lines + hex node polygons), hover card, click-to-allocate (`_allocateNode()`), `getSkillTreeBonuses()` with stat-string parser. `_skillTreeState`, `_stNodeMap`, `_skillTreeVB`, pan/zoom helpers. Full implementation added in v6.90. |
