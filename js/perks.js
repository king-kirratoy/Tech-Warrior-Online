...
const _perks = {
    ...
    mg_velocity:     { cat:'mg', label:'High Velocity',    desc:'MG damage +10% at range >350px',                         apply: () => { _perkState.mgVelocity=true; } },
    ...
    br_reload:       { cat:'br', label:'Fast Hands',       desc:'BR fire rate +25% (stackable)',                          apply: () => { _perkState.reloadMult=(_perkState.reloadMult||1)*0.75; } },
    ...
    hr_reload_boost: { cat:'hr', label:'Quick Cycle',      desc:'HR fire rate +20% (stackable)',                          apply: () => { _perkState.reloadMult=(_perkState.reloadMult||1)*0.80; } },
    ...
    sr_reload:       { cat:'sr', label:'Bolt Action Pro',  desc:'SR fire rate +25% (stackable)',                          apply: () => { _perkState.reloadMult=(_perkState.reloadMult||1)*0.75; } },
    ...
    gl_reload:       { cat:'gl', label:'Autoloader',       desc:'GL fire rate +20% (stackable)',                          apply: () => { _perkState.reloadMult=(_perkState.reloadMult||1)*0.80; } },
    ...
    rl_reload:       { cat:'rl', label:'Rapid Loader',     desc:'RL fire rate +20% (stackable)',                          apply: () => { _perkState.reloadMult=(_perkState.reloadMult||1)*0.80; } },
    ...
    plsm_reload:     { cat:'plsm', label:'Capacitor Bank', desc:'PLSM fire rate +20% (stackable)',                        apply: () => { _perkState.reloadMult=(_perkState.reloadMult||1)*0.80; } },
    ...
    fm_speed:        { cat:'fortress_mode', label:'Rapid Advance',desc:'Fortress Mode does not slow move speed',             apply: () => { _perkState.fmSpeed=true; } },
    ...
    hb_reload:       { cat:'hydraulic_boost', label:'Sprint Fire',   desc:'Fire rate +15% while moving at full speed',          apply: () => { _perkState.hbReload=true; } },
    ...
    ma_reload:       { cat:'mag_anchors', label:'Planted Hands',  desc:'While anchored: fire rate +25%',                       apply: () => { _perkState.maReload=true; } },
    ...
    fw_reload:       { cat:'featherweight', label:'Swift Hands',  desc:'Featherweight: +8% more fire rate (stackable)',       apply: () => { _perkState.reloadMult=(_perkState.reloadMult||1)*0.92; } },
    ...
    sb_reload:       { cat:'sprint_boosters', label:'Sprint Reset', desc:'Sprint burst instantly resets both weapon fire delays', apply: () => { _perkState.sbReload=true; } },
    ...
    rl2_reload:      { cat:'reactor_legs', label:'Sprint Fire',  desc:'Reactor Legs: fire rate +15% while moving',           apply: () => { _perkState.rl2Reload=true; } },
    ...
    sl_reload:       { cat:'suppressor_legs', label:'Pressure Cooker',desc:'While enemies are in your aura: fire rate +20%',  apply: () => { _perkState.slReload=true; } },
